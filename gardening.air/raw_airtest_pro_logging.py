# -*- encoding: utf-8 -*-
"""
raw_airtest_pro_logging.py
Logging & Report-System für raw_airtest_pro.
- Zeitgestempelte Screenshots
- JSON-Log
- HTML-Report mit Screenshots
- Tap/Swipe mit Screenshot
"""
...
import json
import time
from raw_airtest_pro import raw_screenshot, find_template, all_matches_raw, tap, swipe, exists_raw
...
# ------------------ Exists mit Screenshot-Logging ------------------
def exists_raw_logged(tpl, viewport=None, timeout=2):
    start = time.time()
    while time.time() - start < timeout:
        path, vp = save_screenshot_with_timestamp("exists", viewport)
        match = find_template(tpl, path)
        if match:
            log_action("exists", template_name=tpl.filename, position=match["result"],
                       confidence=match["confidence"], extra={"screenshot": path})
            return match["result"]
        time.sleep(0.2)
    log_action("exists", template_name=tpl.filename, position=None, confidence=None)
    return None

# ------------------ All Matches mit Screenshot-Logging ------------------
def all_matches_raw_logged(tpl, viewport=None):
    path, vp = save_screenshot_with_timestamp("allmatches", viewport)
    matches = all_matches_raw(tpl, viewport)
    log_action("all_matches", template_name=tpl.filename, extra={"matches": matches, "screenshot": path})
    return matches

# ------------------ Tap/Swipe Logging mit Screenshot ------------------
def tap_logged(pos, device_addr=None):
    # device_addr nur zur Kennzeichnung, nicht pos
    device_str = str(device_addr) if device_addr else "unknown"
    screenshot, _ = save_screenshot_with_timestamp(f"tap_{device_str}")
    tap(pos)
    log_action("tap", position=pos, extra={"screenshot": screenshot, "device": device_addr})


def swipe_logged(start, end, duration=0.5):
    screenshot, _ = save_screenshot_with_timestamp("swipe")
    swipe(start, end, duration)
    log_action("swipe", extra={"start": start, "end": end, "duration": duration, "screenshot": screenshot})
...
def drag_seed_logged(seed_tpls, free_place_tpl, viewport=None):
    ...
    for seed_tpl in seed_tpls:
        ...
        if seed:
            ...
            log_action("drag_seed", template_name=seed_tpl.filename,
                       extra={"to_slot": free_place_tpl.filename})
            return True
    return False
