...
Multithread-fähig, globales Android SDK ADB.
"""
import os
import subprocess
import time
...
def get_display_info(device_addr):
    """Liest aktuelle Anzeigegröße und DPI aus dem Emulator"""
...
def estimate_viewport(screenshot_path, app_ratio=(9, 16)):
    """Schätzt automatisch den sichtbaren App-Bereich anhand Seitenverhältnis"""
    ...
    right = left + app_w
    bottom = top + app_h
    print(f"[INFO] Geschätzter App-Viewport: ({left},{top},{right},{bottom})")
    return (left, top, right, bottom)


# ------------------ Screenshot ------------------
def raw_screenshot(device_addr, filename="raw.png", crop=True, viewport=None):
    """Unskalierter Screenshot vom Emulator"""
